...
casper.test.begin('Test that products can be added to cart', function suite(test) {
    var config = {
        baseUrl: baseUrl,
        productData: {},
    ...
    if (!args['skip-home'] && ['top-category', 'category', 'product'].indexOf(pathOptions.key) < 0) {
        casper.then(function() {
            ...
            require('tests/ui/home')(test, this, defaults);
        });
    }
    ...
    if (!args['skip-top-category'] && ['category', 'product'].indexOf(pathOptions.key) < 0) {
        casper.then(function() {
            ...
            require('tests/ui/top-category')(test, this);
        });
    }
    ...
    if (!args['skip-category'] && ['product'].indexOf(pathOptions.key) < 0) {
        casper.then(function() {
            ...
            require('tests/ui/category')(test, this, config);
        });
    }
    ...
    if (!args['skip-product']) {
        casper.then(function() {
            ...
            config.productData = require('tests/ui/product')(test, this, config);
        });
    }
    ...
    if (!args['skip-cart']) {
        casper.then(function() {
            ...
            require('tests/ui/cart')(test, this, config);
        });
    }
